...
qtpath = "E:\Projects\Discord bot\QTBOT\qtbot"
...
image_directory = "E:\Projects\Discord bot\QTBOT\qtbot\girldatabase\images"
...
class qt_battle():
    ...
    def vote_girl(self, girl, author):
        ...
        if author in self.voters:
            ...
            if self.voters[author] is 2:
                ...
                self.vote_A -= 1
        exec("self.vote_{0} += 1".format(letter))
    ...
    def end(self):
        ...
        if not draw:
            ...
            return '{0} Wins! Her rating is now {1}(+{2})'.format(self.girls[winner - 1], self.girls[winner - 1].elo, (self.girls[winner - 1].elo-oldELO))
        else:
            ...
            return "It's a tie! QTR change: {} ({})  {} ({})".format(self.girls[0], (self.girls[0].elo - old_ELO_A), self.girls[1],(self.girls[1].elo - old_ELO_B))
...
async def on_message(message):
    if ((message.content.startswith('>qtbattle')
        ...
        #message.channel needs to be in battles_ongoing ASAP to prevent multiple qtbattles in the same channel,
        #otherwise a >qtb message sent while the images are uploading would start another one causing all sorts of headache.
        print('Starting QT battle')
 
        battle.caller = message.author
        tag_regex = re.compile('((((?<=>qtb\s)|(?<=>qtbattle\s))([a-zA-Z0-9-0_]+))|((?<=,)([a-zA-Z0-9-0_]+)))')
        ...
        if message.content == ('>qtb') or message.content == ('>qtbattle'):
            ...
            if (len(all_girls) >= 2):
                ...
                tempmessage = await client.send_message(message.channel, "Couldn't find enough girls with provided tags, using random girls")
                all_girls = QtAnimeGirl.objects.all()
 
        battle.girls = random.sample(list(all_girls), 2)

        #TODO: Make print for battle result
        print('Girl 1 = {}({})'.format(battle.girls[0].id,battle.girls[0].elo))
        print('Girl 2 = {}({})'.format(battle.girls[1].id,battle.girls[1].elo))
        print('--------------')
 
        await client.send_typing(message.channel)
        image_girl_a = await client.send_file(
            message.channel,
        ...
        await client.send_typing(message.channel)
        image_girl_b = await client.send_file(message.channel,
        ...
        battle.vote = await client.send_message(message.channel, '%s vs %s - 0 - 0' % (battle.girls[0], battle.girls[1]))
    ...
    if message.channel in battles_ongoing:
        ...
        if (message.content.startswith('>name')):
            ...
            new_name = re.search('(?<=\>name\s{0}\s).+'.format(girl), message.content).group()
            battle.name(girl, new_name)
            await client.send_message(message.channel, 'Girl {0} now known as {1}!'.format(battle.girls[girl - 1].id,battle.girls[girl - 1]))
        ...
        if message.content.startswith('>vote'):
            ...
            girl = int(message.content[6])
            battle.drop(girl)
            await client.send_message(message.channel, 'Girl {} has been dropped from the database!'.format(battle.girls[girl-1]))
            ...
            and ((message.author == battle.caller)
            or (message.author == owner_id))):
    ...
    if message.content.startswith('>bestgirl'):
        ...
        tag_regex = re.compile('(((?<=-randqt\s)([a-zA-Z0-9-0_]+))|((?<=,)([a-zA-Z0-9-0_]+)))')
    ...
    # This should probably not be in the qtbattle file :-).
        ...
        search_param = re.search('(?<=-rand\s)[0-9]+',message.content)
        try:
            param = int(search_param.group())
            mention = message.author.mention
            await client.send_message(message.channel,'{} rolled **{}**'.format(mention,random.randint(0,param)))
        except:
            pass
 
    elif (message.content.startswith('>setgame') and (message.author == owner_id)):
        search_param = re.search('(?<=\>setgame\s).+',message.content)
    ...
    if ':B1:' in message.content:
        await client.send_file(message.channel,open('misc files\B1.png','rb'),filename = 'B1.png')
    if ':B2:' in message.content:
        await client.send_file(message.channel,open('misc files\B2.png','rb'),filename = 'B2.png')
    if ':sectoid:' in message.content:
        await client.send_file(message.channel,open('misc files\-sectoid-.png','rb'),filename = '-sectoid-.png')
